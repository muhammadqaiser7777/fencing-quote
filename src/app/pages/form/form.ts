...
export class Form implements OnInit {
  ...
  validateCurrentStep(): boolean {
    ...
    if (this.currentStep === 1) {
      if (!this.Fencingtype) {
        this.errors['Fencingtype'] = 'Please select a service.';
        valid = false;
      }
      ...
      if (!this.BriefRequirement.trim()) {
        this.errors['BriefRequirement'] = 'Please provide a brief description of your fencing remodeling needs.';
        valid = false;
      }
  ...
  async submit() {
    ...
    if (this.validateCurrentStep()) {
      ...
      const payload = {
        Fencingtype: parseInt(this.Fencingtype),
      ...
      this.http.post('https://get-fencing.com/server/forward-lead', payload).subscribe({
        next: (response) => {
